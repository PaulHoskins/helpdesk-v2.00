...
DEFINE BUFFER b-query  FOR ptp_proj.
DEFINE BUFFER b-search FOR ptp_proj.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Project Templates") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webprojtemp.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Project Templates") skip.

    {&out}
    tbar-Begin(
        tbar-Find(appurl + "/sys/webprojtemp.p")
        )
    tbar-Link("add",?,appurl + "/sys/webprojtempmnt.p",lc-link-otherp)
    tbar-BeginOption()
    tbar-Link("view",?,"off",lc-link-otherp)
    tbar-Link("update",?,"off",lc-link-otherp)
    tbar-Link("delete",?,"off",lc-link-otherp)
    tbar-Link("phase",?,"off",lc-link-otherp)
    tbar-EndOption()
    tbar-End().
    {&out} skip
    ...
    {&out}
    htmlib-TableHeading(
        "Template Code^left|Description^left"
        ) skip.
        ...
        ELSE
            ...
            DO:
                FIND FIRST b-search 
                    ...
                    AND b-search.projcode >= lc-search NO-LOCK NO-ERROR.
    ...
    REPEAT WHILE AVAILABLE b-query:
        ...
        ASSIGN 
            lc-link-otherp = 'search=' + lc-search +
                             ...
                             '&projectcode=' + string(b-query.ProjCode).
        ...
        {&out}
            ...
            skip
            htmlib-MntTableField(html-encode(b-query.ProjCode),'left')
            htmlib-MntTableField(html-encode(b-query.Descr),'left')
              tbar-BeginHidden(rowid(b-query))
                tbar-Link("view",rowid(b-query),appurl + "/sys/webprojtempmnt.p",lc-link-otherp)
                tbar-Link("update",rowid(b-query),appurl + "/sys/webprojtempmnt.p",lc-link-otherp)
                tbar-Link("delete",rowid(b-query),
                          if DYNAMIC-FUNCTION('com-CanDelete':U,lc-user,"webprojtemp",rowid(b-query))
                          then ( appurl + "/sys/webprojtempmnt.p") else "off",
                          lc-link-otherp)
                tbar-Link("phase",rowid(b-query),appurl + "/sys/webprojphase.p",lc-link-otherp + "&init=yes"     
                                                )
    ...
    {lib/navpanel.i "sys/webprojtemp.p"}
