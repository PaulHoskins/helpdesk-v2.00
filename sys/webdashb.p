...
DEFINE BUFFER b-query  FOR dashb.
DEFINE BUFFER b-search FOR dashb.
...
------------------------------------------------------------------------------*/
    ...
    {&out} htmlib-Header("Maintain Dashboard") skip.

    {&out} htmlib-JScript-Maintenance() skip.

    {&out} htmlib-StartForm("mainform","post", appurl + '/sys/webdashb.p' ) skip.

    {&out} htmlib-ProgramTitle("Maintain Dashboard") skip.
    

    {&out}
    tbar-StandardBar(
        appurl + "/sys/webdashb.p",
        appurl + "/sys/webdashbmnt.p",
        lc-link-otherp
        ).
    ...
    {&out}
    htmlib-TableHeading(
        "Dashboard Code|Description|Active?"
        ) skip.
        ...
        ELSE
            ...
            DO:
                FIND FIRST b-search 
                    ...
                    AND b-search.dashcode >= lc-search NO-LOCK NO-ERROR.
    ...
    REPEAT WHILE AVAILABLE b-query:
        ...
        {&out}
            ...
            htmlib-MntTableField(html-encode(b-query.dashCode),'left')
            htmlib-MntTableField(html-encode(b-query.descr),'left')
            htmlib-MntTableField(html-encode((if b-query.isActive = true
